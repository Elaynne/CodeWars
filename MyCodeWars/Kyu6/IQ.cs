...
namespace MyCodeWars
{
    public class IQ
    {
        ...
        */
        ...
        {
            List<string> strList = numbers.Split(' ').ToList();

            return strList.IndexOf(strList.GroupBy(n => Convert.ToInt32(n) % 2 == 0).Where(n => n.Count() == 1)
              .Select(n => n.ElementAt(0)).FirstOrDefault()) + 1;
        }
    }
}