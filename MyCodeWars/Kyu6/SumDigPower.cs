...
namespace MyCodeWars
{
    public class SumDigPower
    {
        ...
        */
        ...
        {
            long tNumbers = b - a;
            List<long> eurekaNumbers = new List<long>();

            for (long i = 0; i <= tNumbers; i++)
            {
                var numList = (a + i).ToString().ToList().Select(c => c.ToString()).ToList();
                long soma = 0;
                for (int j = 0; j < numList.Count(); j++)
                {
                    soma += (long)System.Numerics.BigInteger.Pow(Convert.ToInt32(numList[j]), j + 1);
                }

                if ((a + i) == soma)
                    eurekaNumbers.Add(a + i);
            }
            return eurekaNumbers.ToArray();
        }
    }
}
